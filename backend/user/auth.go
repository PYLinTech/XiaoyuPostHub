...
//
//   - 账号不存在 → 创建(groups = ['user']),不写 'all'
//   - 账号存在 + 哈希不一致 → UPDATE password_hash 直接覆盖
//   - 账号存在 + 哈希一致 → 不动
//
// 运行时对 EnvSuperAdmin 的 'all' 临时附加由 Repo.GetByUsername 处理。
...
// HealAllGroup 启动期调用一次,清理库中残留的 'all'(篡改/脏数据兜底)。
// 正常流程下 CHECK no_all 已经保证不会写入 'all',这里兜历史数据。
func HealAllGroup(ctx context.Context, q *sqlcgen.Queries) error {
	affected, err := q.RemoveAllFromAllUsers(ctx)
	if err != nil {
		return fmt.Errorf("自愈清理失败: %w", err)
	}
	if affected > 0 {
		log.Printf("WARN: 检测到 %d 个用户含 'all' 组(篡改或脏数据),已自动清除", affected)
	}
	return nil
}
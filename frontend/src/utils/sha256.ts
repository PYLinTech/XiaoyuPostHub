...
class IncrementalSHA256 {
  ...
  digest() {
    ...
    const lengthOffset = this.buffered < 56 ? 56 : 120;
    const high = Math.floor(this.bytes / 0x20000000);
    const low = (this.bytes * 8) >>> 0;
    const view = new DataView(tail.buffer);
    view.setUint32(lengthOffset, high, false);
    view.setUint32(lengthOffset + 4, low, false);
    this.transform(tail.subarray(0, 64));
    if (lengthOffset === 120) this.transform(tail.subarray(64, 128));
